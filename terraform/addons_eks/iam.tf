data "aws_iam_policy_document" "aws_cli_describe_ec2_instances_assume_policy" {
  statement {
    ...
    principals {
      type = "Federated"
      identifiers = [
        data.terraform_remote_state.layer_terraform_eks.outputs.oidc_provider_arn
      ]
    }
    actions = [
    ...
    condition {
      test     = "StringEquals"
      variable = "${data.terraform_remote_state.layer_terraform_eks.outputs.oidc_provider_url_without_http_prefix}:aud"
      values   = ["sts.amazonaws.com"]
    }
    condition {
      test     = "StringEquals"
      variable = "${data.terraform_remote_state.layer_terraform_eks.outputs.oidc_provider_url_without_http_prefix}:sub"
